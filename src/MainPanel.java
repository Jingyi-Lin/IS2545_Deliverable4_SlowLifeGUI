...
public class MainPanel extends JPanel {
    ...
    /*
     * MainPanel.converToInt() is used by getNumNeighbors(),
     * since the return code in getNumNeighbors() has been modified,
     * actually after modification this method is not in use.
     */
    ...
    /*
     * Modify MainPanel.getNumNeighbors() to make it more efficient,
     * for easy testing, change the private method to public
     */ 
    ...
    public int getNumNeighborsRetired(int x, int y) {
	int size = _size;
	int leftX = (x - 1) % size;
	int rightX = (x + 1) % size;
	int upY = (y - 1) % size;
    ...
    /*
     * Modify MainPanel.backup() to be more efficient,
     * put initialize code into MainPanel().
     */
    public void backup() {
       for (int j = 0; j < _size; j++) {
	    for (int k = 0; k < _size; k++) {
     ...
     */
     ...
     /*
     * Modify MainPanel.runContinuous() to make it more efficient
     */
    ...
    public void runContinuousRetired() {
	_running = true;
	while (_running) {
	    System.out.println("Running...");
	    int origR = _r;
    ...
    /* 
     * Add initializing code of _backupCells, make code more efficient.
    */
	...
	_cells = new Cell[size][size];
        _backupCells = new Cell[_size][_size];  // modified
	for (int j = 0; j < size; j++) {
	    for (int k = 0; k < size; k++) {
		_cells[j][k] = new Cell();
                _backupCells[j][k] = new Cell();    // modified
		this.add(_cells[j][k]);
		_cells[j][k].setAlive(false);
	    }
	}
